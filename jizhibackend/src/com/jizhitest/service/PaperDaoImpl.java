...
import com.jizhibackend.bean.Concept;
...
public class PaperDaoImpl {
	...
	private  String addConcept= "com.jizhitest.mapping.paperMapping.addConcepts";
...
//	}
	...
	public int addQustionList(Paper paper,List<Question> list)
	{
		int paperid=paper.getId();
		int i = 0;
		List<Concept> list2=new ArrayList<Concept>();
		for(Question q:list)
		{
...
//			System.out.println("顺序："+q.getQ_order());
	...
	public int addChoiceQustion(int paperid,SingleChoiceQuestion scq)
	{
		int i=0;
		Question q=new Question();
		Concept c=new Concept();
		c.setConcept(scq.getTag());
		q.setType(1);
		...
		if(addQuestion(paperid, q,scq.getQ_order(),c)!=0)
		{
			Connection conn=DBConn.getConnection();		   
			try {
	...
	public int addChoiceQustion(int paperid,MultipleChoiceQuestion scq)
	{
		int i=0;
		Question q=new Question();
		Concept c=new Concept();
	...
	public int addQuestion(int paperid,Question q,int order,Concept c)
	{
		int i=0;
		i=session.insert(addQStmt, q);
		int j=0;
	...
	public int changeShare(int isShare,int userid,int paperid){
